...
void ControllerCore::updateStatus(SDcardHandler &SDcard) {
...
void ControllerCore::updateSerial(SerialHandler &USB, SDcardHandler &SDcard) {
  ...
  if (newChecks != WaterLS.getSensorChecks()){
    ...
    if (!openTime && SDcard.available()) SDcard.saveParam(newChecks, newLimit, newFlow);
  }
...
void ControllerCore::pumpCore(SDcardHandler &SDcard) {
  ...
  if (coreLoop == CORE_LOOPS) {                                   // Igual que en "checkSwitchMode()" esta rutina se ejecuta sólo cada CORE_LOOP ciclos.
    ...
    if (Switches.getSwitch(1)){                                   //Modo de control según el botón B1: 1 = Auto; 0 = Manual.
      ...
      if (WaterLS.getWaterLevel() == underMin && !Pump.getPump()) Pump.pumpON(SDcard, LEDs);      // Si el depósito está vacío Y la bomba no está ya encendida, la enciende.
      else if (WaterLS.getWaterLevel() == overMax && Pump.getPump()) Pump.pumpOFF(SDcard, LEDs);  // Si el depósio está lleno Y la bomba no se ha apagado ya, la apaga.
    }
    else if (Switches.getSwitch(2) && !Pump.getPump()) Pump.pumpON(SDcard, LEDs);   // Si el botón está en  "ENCENDIDO" y la bomba no está ya encendida, la enciende.
    else if (!Switches.getSwitch(2) && Pump.getPump()) Pump.pumpOFF(SDcard, LEDs);  // Si el botón está en la posición "APAGADO" y la bomba sigue encendida, la apaga.
  }
  else coreLoop++;
}
