...
void controlCore(bool mode){                                            
  ...
  if (coreLoop == CORE_LOOP) {                                    // Igual que en "checkSwitchMode()" esta rutina se ejecuta sólo cada CORE_LOOP ciclos.
    FCN
    coreLoop = 0;
    if (mode){                                                    // Modo de control según el botón switchMode: 1 = Auto; 0 = Manual.
      ...
      checkSwitchPump();                                          // Controla el estado del botón de encendido manual de la bomba.
