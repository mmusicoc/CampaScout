...
Adafruit_PCD8544 display = Adafruit_PCD8544( 5, 4, 3); //Reset display, data display, data display)

void setup() {
  pinMode(2, INPUT_PULLUP); //Right button
  pinMode(1, INPUT_PULLUP); //Middle button
  pinMode(0, INPUT_PULLUP); //Left button
  pinMode(7, OUTPUT); // Backlight
...
void loop() {
  ...
  if (middle){
    ...
    if (page == 1 && menuitem == 3)resetDefaults();
    else if (page == 1 && menuitem == 1) page = 2;
...
void checkIfDownButtonIsPressed() {
  if (downButtonState != lastDownButtonState) {
    if (downButtonState == 0) down = true;
    delay(50);
  }
...
void checkIfSelectButtonIsPressed(){
  if (selectButtonState != lastSelectButtonState){
    if (selectButtonState == 0) middle = true;
    delay(50);
  }
